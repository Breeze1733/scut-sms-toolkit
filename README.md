...
导出的 Excel 表格会自动生成三个独立的工作表（Sheet），方便直接对照核对：

| 工作表名称 (Sheet) | 包含课程类型 | 排除课程类型 | 表头格式 |
| :--- | :--- | :--- | :--- |
| **班级通过** | 仅限状态或该行包含“班级通过”的课程（班级评议小组审核通过的课程） | 未通过或非“班级通过”状态的课程 | 姓名、加权平均分、学分总数 |
...
| **学业成绩积分 (X)** | 班级通过课程学业加权平均分 | 取功能二 Sheet 1“班级通过”课程加权均分 |
...
| **德育积分** | 德育模块加分 | 读取 `0301 德育加分` 模块的“学生申请分数”列求和（**封顶 25 分**） |
...
> 💡 **提示与规则**：
> 1. **德育积分封顶**：按照华工综测细则，学生申请的德育加分项求和后会自动执行 **最高 25 分封顶** 处理后再计入综合素养总分 $S$；
> 2. **文件命名规则**：导出的 Excel 与 ZIP 文件命名均自动加入本地精准时间戳（格式为 `_YYYY-MM-DD_HH-mm`，即包含小时与分钟），避免多次导出时文件名覆盖。
