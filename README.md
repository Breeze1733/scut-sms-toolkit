...
在左侧菜单进入“智育测评”或班级综测数据审核列表页，确保当前处于包含全班学生列表的表格界面（每行包含“查看”操作链接）。
...
* **计算公式**：$\text{加权均分} = \frac{\sum(\text{成绩} \times \text{学分})}{\sum \text{学分}}$，四舍五入保留 2 位小数。
...
适配者可以 Fork 本仓库后针对本学院的情况修改逻辑，也欢迎提 Pull Request 合并进主仓库，方便更多华工校友使用！
